...
The one dangerous failure here would be a hardware path that derives a
*different* key, so item 5 (an old sealed blob still opening) is the gate. Do
not sign off this section without it, and do not run any of it on a board whose
seed you cannot re-provision.

1. **Timed unseal, per slot, before and after.** Flash the previous release,
   ...
   the path and item 2's log line will say so. Record both numbers for the V4,
...
2. **The self-check ran and passed.** On a bench build with the console on a
   ...
   Without a console, item 1's timing is the observable.
3. **A three-master unlock.** On a board with three sealed masters, unlock once
   and confirm: all three slots open on one PIN entry, the OLED progress card
   counts `1/3`, `2/3`, `3/3`, the total is roughly three times item 1's
   ...
   stretches, not three.
4. **Unseal while a relay session is live on WiFi.** This is the contention
   case the shared SHA/AES lock creates, and it only exists in the
   WiFi-standalone tier. Provision a board for WiFi-standalone with a vault
   key, reboot it locked, and let it come up and announce its ephemeral unlock
   pubkey (kind 24135) — the TLS session to the relay is live at that moment.
   Deliver the vault key with `node scripts/vault-unlock-wifi.mjs` and confirm:
   the device unlocks, the relay session does NOT drop or error, a `sign_event`
   sent immediately afterwards is answered normally, and the next boot reports
   no crash crumb. Then do it the other way round: with the board already
   unlocked and a relay session established, trigger a fresh `derive_km` (a
   `VAULT_SET`/PIN change, which reseals every slot) while an app is signing
   through the relay. Neither side may stall for more than a second or two, and
   nothing may be signed wrongly. If the log shows `SHA accelerator contended -
   finishing this derivation in software`, that is the retreat working, not a
   failure; note how often it fires.
5. **An old sealed blob still opens.** THE GATE. Take a board sealed by a
   PREVIOUS release — ideally one sealed months ago, and at minimum one sealed
   by the release under test in item 1's "before" half, which must not be
...
6. **The self-check mismatch path.** Build with the bench-only feature that
   ...
   across boots. Then reflash the normal image and confirm item 1's timing
   comes back.
7. **The iteration ceiling.** `MAX_PBKDF2_ITERATIONS` is now 150,000 rather
   ...
   unlock must fail FAST — within the usual frame timeout, not after minutes —
   with the blob refused before any KDF work, and the board must not reboot.
   The failed-attempt counter behaviour is unchanged (this is a malformed
   blob, and it still burns an attempt, as it did before).
8. **Nothing else moved.** Sanity-sweep the paths that share the KDF: enable
