...
| `dk_sec`, `dk_ph`, `lk_boots`, `ann_op`, `ph_relays` | Phone unlock: the data key's PIN/vault wrapper, the packed phone records, the locked-restart count, the operator-announcement switch, and `ph_relays`, the relays the phones were last told about with the count of relay-update rounds sent (written when first needed, when a change only dropped relays, after each of a relay change's six update rounds, the last of which records the new list, and replaced at the first enrolment after every phone has gone; never on a network-trial boot; removed with the last phone) |
