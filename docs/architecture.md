...
- **Connected to a WiFi signer by address** (including from a phone in another country). Sapwood signs encrypted kind-24134 requests with the provisioned operator key and exchanges them through the signer's configured relays. Client policy, staged network mutations and unlock phones (list, revoke, and adding one, which still needs a press on the board) are remote; seed/PIN/trust-root changes, factory reset, and OTA are not.
