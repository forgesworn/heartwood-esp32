...
- **Connected to a WiFi signer by address** (including from a phone in another country). Sapwood signs encrypted kind-24134 requests with the provisioned operator key and exchanges them through the signer's configured relays. Client policy, staged network mutations and unlock phones (list and revoke) are remote; the firmware also accepts adding a phone over the relay (`enrol_unlock_phone`, still a press on the board), but Sapwood does not offer it yet (a pending follow-up). Seed/PIN/trust-root changes, factory reset, and OTA are not remote.
